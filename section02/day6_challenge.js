...
매개변수 logs로 이벤트 로그 배열을 전달받습니다.
이벤트 로그들 중 type이 "click"에 해당하는 로그만 특별한 형태로 출력합니다
형태: ${이벤트아이디}: ${이벤트발생시각}
...
function printClickEventLogs(logs) {
  ...
  logs.forEach( (event) => {
    if( event.type === "click" ) {
        eventTime = new Date(event.date).toLocaleString();
        // ts = eventTime.toLocaleString('ko-KR', { timeZone: 'UTC' });
        console.log(`${event.type} : ${eventTime}`);
        // console.log(`${event.type} : ${ts}`);
    }
...
카페 메뉴 배열인 menus를 매개변수로 제공받습니다.
menus 배열의 모든 아이템(메뉴)에 기존가(price)에서 500원 할인된 할인가를 저장하는 discountedPrice를 추가한 새로운 배열을 만듭니다.
discountedPrice가 추가된 새로운 배열을 반환합니다.
...
function getDiscountedMenus(menus) {
  ...
  return menus.map( (item) => {
    item.discountedPrice = item.price - 500;
    return item;
  });
}
...
매개변수 books로 도서 배열을 제공 받습니다.
books 배열을 도서의 출간일(published)를 기준으로 내림차순(최신순)으로 정렬합니다
정렬된 배열을 반환합니다.
...
function getSortedBooks(books) {
  // 여기에 코드를 작성하세요 ...
  return books.toSorted( (beforeBook,afterBook) => {
    const beforeBookTime = beforeBook.published.getTime();
    const afterBookTime = afterBook.published.getTime();
    if ( beforeBookTime > afterBookTime ) {
        return -1;
    } else if ( beforeBookTime < afterBookTime ) {
        return 1;
    } else {
        return 0;
    }
